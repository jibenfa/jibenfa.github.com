...
s2.addremove=false
s2.anonymous=true
s2:tab("config3", translate("选择v2ray运行模式"),translate("<p>本模块是选择v2ray的运行模式，支持三种模式：被墙网站白名单模式，境外全局代理模式，墙内上网模式。<p>"))
...
#
# To use this file, install chinadns,v2ray,dnsmasq-full first
#
START=90
...
set_multi_foreigndns() {
    echo "set to foreigndns"
    killall chinadns
    /etc/init.d/chinadns restart
    nohup /usr/bin/chinadns -m -b 0.0.0.0 -p ${chinadns_port_2} -s ${domestic_dns_2},127.0.0.1:${v2_dns_port_2} -c /etc/chinadns_chnroute.txt 1>/dev/null 2>&1 &
    nohup /usr/bin/chinadns -m -b 0.0.0.0 -p ${chinadns_port_3} -s ${domestic_dns_3},127.0.0.1:${v2_dns_port_3} -c /etc/chinadns_chnroute.txt 1>/dev/null 2>&1 &
    nohup /usr/bin/chinadns -m -b 0.0.0.0 -p ${chinadns_port_4} -s ${domestic_dns_4},127.0.0.1:${v2_dns_port_4} -c /etc/chinadns_chnroute.txt 1>/dev/null 2>&1 &
    ...
    uci add_list dhcp.@dnsmasq[0].server=127.0.0.1#${chinadns_port_2}
    uci add_list dhcp.@dnsmasq[0].server=127.0.0.1#${chinadns_port_3}
    uci add_list dhcp.@dnsmasq[0].server=127.0.0.1#${chinadns_port_4}
...
enable_firewall_rules() {
    ...
    if [ "$v2ray_mode" = "gfwlist" ]; then
        ...
        if [ x${localip} != x${current_dns_list:0:9} ]; then
            set_multi_foreigndns
        fi
    elif [ "$v2ray_mode" = "outlands" ]; then
        ...
        echo "ingfw mode" > /tmp/v2raymode.txt
        iptables -t nat -D PREROUTING -p tcp -m set ! --match-set bypasslist dst -j REDIRECT --to-port 1060
        iptables -t nat -D OUTPUT -p tcp -m set ! --match-set bypasslist dst -j REDIRECT --to-port 1060
        iptables -t nat -D PREROUTING -p tcp -m set --match-set gfwlist dst -j REDIRECT --to-port 1060
        iptables -t nat -D OUTPUT -p tcp -m set --match-set gfwlist dst -j REDIRECT --to-port 1060
