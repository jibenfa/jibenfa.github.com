...
<a href="http://wallsee.org/2019/06/14/v2ray2bLetsEncrypt2bcdne983a8e7bdb2.html">配置</a>

2.chinadns在openwrt的配置参考：

<a href="https://wallsee.org/2019/06/09/v2raye59ca8openwrte4b88be79a84e5ae89e8a385e983a8e7bdb2.html">配置的“8）安装和配置chinadns”小节</a>
...
s2.addremove=false
s2.anonymous=true
s2:tab("config3", translate("编辑v2ray规则文件"),translate("<p>本页是编辑v2ray规则，支持三种模式：白名单模式，境外全局代理模式，墙内上网模式。<p>"))
...
enable_firewall_rules() {
    ...
    if [ "$v2ray_mode" = "gfwlist" ]; then
        ...
        if [ x${localip} != x${current_dns_list:0:9} ]; then
            ...
            uci add_list dhcp.@dnsmasq[0].server=127.0.0.1#5355
            uci add_list dhcp.@dnsmasq[0].server=127.0.0.1#5356
            uci add_list dhcp.@dnsmasq[0].server=127.0.0.1#5357
            uci commit dhcp
