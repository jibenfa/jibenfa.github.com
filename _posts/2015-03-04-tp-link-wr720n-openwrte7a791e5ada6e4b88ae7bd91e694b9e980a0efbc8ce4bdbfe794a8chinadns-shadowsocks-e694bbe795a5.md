...
<pre class="lang:sh decode:true ">satisfy_dependencies_for: Cannot satisfy the following dependencies for kmod-usb-storage:
kernel (= 3.3.6-1-06e8dc0e21c6891eb05dce4475cadf2b) * kernel (= 3.3.6-1-06e8dc0e21c6891eb05dce4475cadf2b) * kernel (= 3.3.6-1-06e8dc0e21c6891eb05dce4475cadf2b) * kernel (= 3.3.6-1-06e8dc0e21c6891eb05dce4475cadf2b) *</pre>
...
mtd -r write openwrt-ar71xx-generic-tl-wr720n-v3-squashfs-sysupgrade.bin firmware</pre>
...
<pre class="lang:sh decode:true ">telnet 192.168.1.1</pre>

<pre class="lang:sh decode:true " >BusyBox v1.22.1 (2014-09-20 22:01:35 CEST) built-in shell (ash)
...
<pre class="lang:sh decode:true ">vi /etc/opkg.conf</pre>

将src/gz开头的修改为：

<pre class="lang:vim decode:true ">src/gz barrier_breaker_base http://192.168.1.100/base
...
src/gz barrier_breaker_oldpackages http://192.168.1.100/oldpackages</pre>

保存后退出。  
继续运行：

<pre class="lang:sh decode:true ">opkg update
opkg install kmod-usb-ohci kmod-usb2 kmod-fs-ext4 kmod-usb-storage block-mount 	kmod-nls-base kmod-nls-cp437 kmod-ipt-nat-extra iptables-mod-nat-extra</pre>
...
<pre class="lang:sh decode:true ">ls /dev/sda*
</pre>

如果显示了

<pre class="lang:sh decode:true ">/dev/sda   /dev/sda1  /dev/sda2</pre>
...
<pre class="lang:sh decode:true ">mkdir /mnt/sda1
...
umount /dev/sda1
umount /tmp/cproot
echo option force_space &gt;&gt; /etc/opkg.conf</pre>
...
opkg install luci-i18n-chinese</pre>

接着去谷歌一下如下4个软件，下载并放到http文件夹的extra目录下，并安装，注意，这几个文件都不是kernel depend的，所以可以装最新版本：

<pre class="lang:sh decode:true ">opkg install http://192.168.1.100/extra/ChinaDNS_1.3.0-1_ar71xx.ipk
...
opkg install http://192.168.1.100/extra/luci-app-shadowsocks-spec_1.3.0-1_all.ipk</pre>
...
<pre class="lang:sh decode:true " >ln -s /usr/lib/libpolarssl.so /usr/lib/libpolarssl.so.7</pre>

最后更新一下过滤文件，ssh登陆192.168.1.1，运行：

<pre class="lang:sh decode:true ">opkg update
opkg install libcurl curl
curl 'http://ftp.apnic.net/apnic/stats/apnic/delegated-apnic-latest' | grep ipv4 | grep CN | awk -F\| '{ printf("%s/%d\n", $4, 32-log($5)/log(2)) }' &gt; /etc/shadowsocks/ignore.list</pre>

更新完成后该文件有5000多行，至此，所有配置完成，路由器实现了动态科学上网，国内网站不用代理，国外网站使用代理，其原理我画了一张图：

[<img class="alignnone size-full wp-image-183" src="https://github.com/jibenfa/jibenfa.github.com/tree/master/uploads/2015/03/microMsg.1425433929069.jpg" alt="microMsg.1425433929069" width="3744" height="2102" srcset="https://github.com/jibenfa/jibenfa.github.com/tree/master/uploads/2015/03/microMsg.1425433929069.jpg 3744w, https://github.com/jibenfa/jibenfa.github.com/tree/master/uploads/2015/03/microMsg.1425433929069-300x168.jpg 300w" sizes="(max-width: 3744px) 100vw, 3744px" />](https://github.com/jibenfa/jibenfa.github.com/tree/master/uploads/2015/03/microMsg.1425433929069.jpg)
