...
<pre><code class="language-sh">physdiskwrite.exe -u openwrt-x86-64-combined-ext4.img</code></pre>
...
<pre><code class="language-sh">eth0 entered promiscuous mode  
br-lan link is not ready</code></pre>
...
<pre><code class="language-sh">telnet 192.168.1.1
</code></pre>

运行passwd改个密码，然后运行df -m会看见：

<pre><code class="language-sh">Filesystem           1M-blocks      Used Available Use% Mounted on
rootfs                  241382         9    241357   0% /
/dev/root               241382         9    241357   0% /
tmpfs                     1815         0      1814   0% /tmp
tmpfs                        1         0         1   0% /dev</code></pre>

一个字，爽~~~

c)这时候如果运行ifconfig会发现只有一个eth0,这是因为PCIE的网卡驱动没装。。不过这没有关系，我在win7机器上用EasyWebSvr-v1.9搭了一个op源，所以要修改更新源，并安装驱动。

<pre><code class="language-sh">vi /etc/opkg.config</code></pre>

<pre><code class="language-vim">src/gz chaos_calmer_base http://192.168.1.150/packages/base
...
<pre><code class="language-sh">root@OpenWrt:~# ifconfig
...
lo        Link encap:Local Loopback
          ...
          collisions:0 txqueuelen:0
          RX bytes:23183 (22.6 KiB)  TX bytes:23183 (22.6 KiB)</code></pre>

root@OpenWrt:~# vi /etc/config/network

<pre><code class="language-sh">config interface 'loopback'
        option ifname 'lo'
        option proto 'static'
