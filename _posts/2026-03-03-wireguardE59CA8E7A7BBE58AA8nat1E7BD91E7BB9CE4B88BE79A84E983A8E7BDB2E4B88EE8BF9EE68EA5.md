...
场景一：wireguard直连openwrt的ipv6地址即可，这里就不多说了，毫无压力。当然光猫上需要把v6的防火墙关闭。
...
6）使用如下python脚本wg.py生成一个wireguard客户端，搭配config.ini配置文件和原版wireguard的windows客户端使用。我是把wg.py打包成一个exe文件和config.ini文件放到同一个目录下使用的。
